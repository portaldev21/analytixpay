...
export async function parseInvoiceWithAI(text: string): Promise<TPdfParseResult> {
  try {
    ...
    if (!apiKey) {
      ...
      return {
        transactions: [],
        error: 'Configuração de IA não encontrada. Configure OPENAI_API_KEY no arquivo .env.local'
      }
    }
...
Retorne APENAS o JSON, sem markdown ou explicações.`
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'Você é um assistente que extrai dados estruturados de faturas de cartão de crédito e retorna APENAS JSON válido.'
        },
        {
          role: 'user',
          content: prompt
    ...
    const transactions: TParsedTransaction[] = extracted.transactions.map(t => ({
      date: t.date,
    ...
    console.log(`Successfully parsed ${transactions.length} transactions via AI`)
    ...
    if (usage) {
      ...
      console.log(`OpenAI usage: ${usage.prompt_tokens} input + ${usage.completion_tokens} output tokens`)
