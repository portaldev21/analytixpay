  ...
  try {
    ...
    try {
      ...
      return {
        data: null,
        error:
          "Limite de uploads excedido. Aguarde alguns minutos e tente novamente.",
        success: false,
      };
    }
    ...
    if (parseResult.error || !parseResult.transactions.length) {
      ...
      return {
        data: null,
        error: parseResult.error || "Nenhuma transação encontrada",
        success: false,
      };
    }
    ...
    if (invoiceError || !invoice) {
      ...
      return {
        data: null,
        error: invoiceError?.message || "Erro ao criar fatura",
        success: false,
      };
    }
 ...
 */
export async function getInvoicesSummary(accountId: string): Promise<
  TApiResponse<
    {
      invoiceId: string;
      period: string;
