...
npm run lint && npm run build       # Full quality check (replaces npm run fix)
...
### ControleFatura Testing Notes

- **Unit tests:** Vitest with co-located `__tests__/` folders (e.g., `src/lib/analytics/__tests__/stats.test.ts`)
- **Linting:** Biome (not ESLint/Prettier)
- **Quality gate:** Run `npm run lint && npm run build` to validate before committing
