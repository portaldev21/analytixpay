...
> **ControleFatura note:** The app uses Next.js route groups: `(auth)/` for public routes
> (login, signup) and `(dashboard)/` for protected routes. The login page is at `/login`
> and uses Supabase Auth with cookie-based sessions refreshed in middleware.
...
setup('authenticate via Supabase Auth', async ({ page }) => {
  // Navigate to ControleFatura login page (route group: (auth)/login)
  await page.goto('/login');

  // Fill Supabase email/password login form
  await page.getByLabel('Email').fill(process.env.TEST_USER_EMAIL || 'test@example.com');
  await page.getByLabel('Senha').fill(process.env.TEST_USER_PASSWORD || 'testpassword');
  await page.getByRole('button', { name: /entrar/i }).click();

  // Wait for Supabase session to be established and redirect to dashboard
  // Middleware refreshes the cookie-based session on navigation
  await page.waitForURL('/dashboard');
  await expect(page.getByRole('navigation')).toBeVisible();

  // Save signed-in state (includes Supabase session cookies)
...
## ControleFatura Route Groups

ControleFatura uses Next.js route groups to separate public and protected areas:

- **`(auth)/`** - Public authentication routes: `/login`, `/signup`
- **`(dashboard)/`** - Protected routes requiring authentication: `/dashboard`, `/invoices`, `/transactions`, `/analytics`, `/budget`, `/settings`
- **Middleware** (`src/middleware.ts`) guards all `(dashboard)/` routes and refreshes Supabase sessions
- **OAuth callback** at `/auth/callback` exchanges code for Supabase session

When writing E2E tests, keep in mind:
- Unauthenticated access to `/dashboard` redirects to `/login`
- After login, users land on `/dashboard`
- The app uses Portuguese UI labels (e.g., "Entrar" for login, "Senha" for password)
